...
export function resolveAgendaTimezone(raw) {
...
export function resolveAgendaColor(raw) {
...
function renderTimezoneOptions(savedTimezone) {
...
export async function loadCalendarTabContent(options = {}) {
    const slug = getSlug();
    if (!slug) {
...
function renderTimelinePreferenceHTML() {
    return `
...
export function bindCalendarTabInteractions(options) {
    ...
    const startOfDayInput = document.getElementById('agendaStartOfDay');
    startOfDayInput?.addEventListener('change', async () => {
    ...
    const colorInput = document.getElementById('agendaColorInput');
