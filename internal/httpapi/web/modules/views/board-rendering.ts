...
export type BoardColumn = { key: string; title: string; color?: string; isDone: boolean; description?: string };
...
function workflowText(key: string, fallback: string, values: Record<string, string> = {}): string {
  const message = hasI18nKey(key) ? t(key, values) : fallback;
  return message.replace(/\{([a-zA-Z0-9_.-]+)\}/g, (match, name) => values[name] ?? match);
}

function workflowDescription(key: string): string {
...
export function buildWorkflowGuideHtml(boardCols: BoardColumn[]): string {
