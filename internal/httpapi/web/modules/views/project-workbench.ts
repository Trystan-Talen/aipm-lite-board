...
export type WorkspaceView = 'overview' | 'requirements' | 'schedule' | 'board';

type Sprint = {
  id: number;
  number?: number;
  name: string;
  plannedStartAt?: number | string;
  plannedEndAt?: number | string;
...
export function getWorkspaceView(slug?: string | null): WorkspaceView {
  try {
    const raw = localStorage.getItem(`${VIEW_KEY}:${slug || 'default'}`);
    if (raw === 'overview' || raw === 'requirements' || raw === 'schedule' || raw === 'board') return raw;
  } catch { /* ignore storage failures */ }
  return 'requirements';
}

export function setWorkspaceView(view: WorkspaceView, slug?: string | null): void {
  try { localStorage.setItem(`${VIEW_KEY}:${slug || 'default'}`, view); } catch { /* ignore */ }
}

export function getCachedSprints(slug: string): Sprint[] {
  return sprintCache.get(slug) || [];
}

export function hasCachedSprints(slug: string): boolean {
  return sprintCache.has(slug);
}

export async function preloadWorkbenchSprints(slug: string): Promise<Sprint[]> {
  if (sprintCache.has(slug)) return sprintCache.get(slug)!;
  try {
    const response = await apiFetch<{ sprints?: Sprint[] } | null>(`/api/board/${slug}/sprints`);
...
export function buildWorkbenchSummary(board: Board): { total: number; done: number; points: number; unassigned: number } {
...
export function exposeWorkbenchState(): void {
  (window as any).__aipmWorkbench = { getWorkspaceView, setWorkspaceView, getCachedSprints, hasCachedSprints };
}

function allTodos(board: Board): Todo[] {
  const result: Todo[] = [];
  for (const todos of Object.values(board.columns || {})) result.push(...(todos || []));
...
function stageName(board: Board, todo: Todo): string {
  const key = todo.columnKey || todo.status || '';
  const column = (board.columnOrder || []).find((item) => item.key === key);
  const fallback: Record<string, string> = { backlog: '需求池', not_started: '待开始', doing: '开发中', in_progress: '开发中', testing: '测试中', done: '已完成' };
  return column?.name || fallback[key] || key || '未设置';
}

function stageColor(board: Board, todo: Todo): string {
  const key = todo.columnKey || todo.status || '';
  return (board.columnOrder || []).find((item) => item.key === key)?.color || '#7187ff';
}

function priority(board: Board, todo: Todo): { label: string; color: string } {
  const item = (board.priorityOrder || []).find((tier) => tier.key === todo.priorityKey);
  if (item) return { label: item.name, color: item.color || '#7187ff' };
  if (todo.priorityKey) return { label: todo.priorityKey, color: '#7187ff' };
  return { label: '未定级', color: '#9aa5b5' };
}

function memberName(members: BoardMember[], id?: number | null): string {
  if (!id) return '未分配';
...
function progress(todos: Todo[]): { done: number; percent: number; points: number } {
...
function renderNav(view: WorkspaceView): string {
...
function renderProjectHeader(_board: Board, view: WorkspaceView): string {
  const meta: Record<WorkspaceView, { title: string }> = {
    overview: { title: '项目总览' },
    requirements: { title: '需求池' },
    schedule: { title: '迭代排期' },
    board: { title: '开发看板' },
  };
  const current = meta[view];
  const action = view === 'schedule'
    ? '<button class="btn btn--ghost" id="aipmRefreshSprints" type="button">刷新迭代</button>'
    : view !== 'board'
      ? '<button class="btn" id="aipmQuickCreate" type="button">＋ 新建需求</button>'
      : '';
  return `<header class="aipm-projectbar"><div class="aipm-projectbar__identity"><h1>${escapeHTML(current.title)}</h1></div><div class="aipm-projectbar__actions">${action}</div></header>`;
}

function renderOverview(board: Board, members: BoardMember[], sprints: Sprint[]): string {
  const todos = allTodos(board);
  const p = progress(todos);
  const p0 = todos.filter((todo) => ['urgent', 'p0'].includes(todo.priorityKey?.toLowerCase() || '')).length;
  const p1 = todos.filter((todo) => ['high', 'p1'].includes(todo.priorityKey?.toLowerCase() || '')).length;
  ...
  return `<div class="aipm-context-strip"><span>当前迭代</span><strong>${active ? escapeHTML(active.name) : '尚未创建'}</strong></div>
...
function renderRequirementTable(board: Board, members: BoardMember[]): string {
  const todos = allTodos(board);
...
function renderRequirementRow(board: Board, members: BoardMember[], todo: Todo): string {
...
function renderSchedule(board: Board, members: BoardMember[], sprints: Sprint[]): string {
  const todos = allTodos(board);
  ...
  const renderGroup = (sprint: Sprint, items: Todo[]) => {
...
export function buildWorkbenchContent(board: Board, view: WorkspaceView, members: BoardMember[], sprints: Sprint[], legacyBoardHtml: string): string {
  if (view === 'board') return legacyBoardHtml;
  if (view === 'overview') return renderOverview(board, members, sprints);
  if (view === 'schedule') return renderSchedule(board, members, sprints);
  return renderRequirementTable(board, members);
}

export function buildProjectWorkbench(options: WorkbenchOptions & { sprints: Sprint[] }): string {
  const { board, view, members, sprints, legacyBoardHtml } = options;
  return `<div class="aipm-workspace" data-aipm-workspace data-aipm-slug="${escapeHTML(options.slug)}"><div class="aipm-shell"><header class="aipm-topbar"><div class="aipm-topbar__brand"><div class="aipm-brand__mark">A</div><div><strong>AIPM 产研看板</strong><span>需求 · 排期 · 进度</span></div></div></header><div class="aipm-app"><aside class="aipm-sidebar"><div class="aipm-sidebar__project"><div class="aipm-project-avatar">${escapeHTML((board.project.name || '项目').slice(0, 1).toUpperCase())}</div><strong>${escapeHTML(board.project.name)}</strong></div>${renderNav(view)}</aside><main class="aipm-workspace__main">${renderProjectHeader(board, view)}<div class="aipm-workspace__content" id="workspaceContent">${buildWorkbenchContent(board, view, members, sprints, legacyBoardHtml)}</div></main></div></div><div class="aipm-drawer-backdrop" id="aipmDrawerBackdrop"></div><aside class="aipm-drawer" id="aipmTodoDrawer" aria-hidden="true"></aside></div>`;
}

function bindTodoOpeners(root: HTMLElement, board: Board, members: BoardMember[], role?: string | null): void {
  if ((root as any)[contentBoundKey]) return;
  root.addEventListener('click', (event) => {
    const target = event.target as HTMLElement | null;
    const trigger = target?.closest<HTMLElement>('[data-aipm-todo]');
    if (!trigger || !root.contains(trigger)) return;
    // Legacy board cards retain their own click/flow handlers.
    if (trigger.closest('.board')) return;
    const localId = Number(trigger.dataset.aipmTodo);
    const todo = allTodos(board).find((item) => item.localId === localId);
    if (todo) openDrawer(todo, board, members, role);
  });
  (root as any)[contentBoundKey] = true;
}

function openDrawer(todo: Todo, board: Board, members: BoardMember[], role?: string | null): void {
...
function bindRequirementFilters(root: HTMLElement): void {
  if ((root as any)[contentBoundKey]) return;
  const apply = () => {
    const query = (root.querySelector<HTMLInputElement>('#aipmRequirementSearch')?.value || '').trim().toLowerCase();
    const priorityValue = root.querySelector<HTMLSelectElement>('#aipmPriorityFilter')?.value || '';
    const stageValue = root.querySelector<HTMLSelectElement>('#aipmStageFilter')?.value || '';
    const unassigned = root.querySelector<HTMLInputElement>('#aipmUnassignedFilter')?.checked;
    let count = 0;
    root.querySelectorAll<HTMLElement>('[data-aipm-row]').forEach((row) => {
  ...
  root.addEventListener('input', (event) => {
    if ((event.target as HTMLElement)?.closest('#aipmRequirementSearch')) apply();
  });
  root.addEventListener('change', (event) => {
    if ((event.target as HTMLElement)?.closest('#aipmPriorityFilter, #aipmStageFilter, #aipmUnassignedFilter')) apply();
  });
  (root as any)[contentBoundKey] = true;
}

export function bindProjectWorkbench(options: WorkbenchOptions & { sprints: Sprint[] }): void {
  exposeWorkbenchState();
  const root = document.querySelector<HTMLElement>('[data-aipm-workspace]');
  if (!root) return;
  if (!(root as any)[boundKey]) {
    root.querySelectorAll<HTMLElement>('[data-aipm-view]').forEach((button) => button.addEventListener('click', () => {
      const view = button.dataset.aipmView as WorkspaceView;
    ...
    (root as any)[boundKey] = true;
  }
  const content = root.querySelector<HTMLElement>('#workspaceContent');
  if (content) {
  ...
  if (refreshSprints && !(refreshSprints as any)[boundKey]) {
    ...
    (refreshSprints as any)[boundKey] = true;
  }
}
